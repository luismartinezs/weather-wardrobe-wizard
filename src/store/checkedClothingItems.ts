...
export const clothingItemsSlice: StateCreator<ClothingItemsState> = (set) => ({
  ...
  setCheckedClothingItems: (checkedClothingItems: ClothingId[]) => set({ checkedClothingItems: checkedClothingItems }),
  checkClothingItem: (index: ClothingId) => set(produce((draft) => {
    if (draft.checkedClothingItems.includes(index)) {
      draft.checkedClothingItems.splice(draft.checkedClothingItems.indexOf(index), 1);
    } else {
