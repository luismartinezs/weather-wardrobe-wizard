...
function getClothingForWeather(weatherTypes: string[]): string[] {
  ...
  if (areArraysOverlapping(weatherTypes, ["Smoke", "Dust", "Sand", "Ash"])) {
...
function getMinimalClothingForTemperature(
  minT: number,
  maxT: number
): string[] {
  const layers = [];
...
export const getFullClothingSuggestions = (
  forecast: WeatherForecast[] | null
): ClothingSuggestionWithId[] | null => {
  if (!forecast) {
    return null;
  }
  const { minTemp, maxTemp, weatherTypes, maxWindSpeed, maxHumidity } =
    reduceForecast(forecast);
...
export const getMinimalClothingSuggestions = (
  forecast: WeatherForecast[] | null
) => {
  if (!forecast) {
    return null;
  }
  const { minTemp, maxTemp, weatherTypes, maxWindSpeed } =
    reduceForecast(forecast);
