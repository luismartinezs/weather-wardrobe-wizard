...
export function getRecentLocationRef<T extends DocumentData = UserLocationData>(
  documentId: string
) {
...
function addUniqueLocation(
  locations: LocationSuggestion[],
  newLocation: LocationSuggestion
): LocationSuggestion[] {
...
export async function addRecentLocation(
  ...
  if (userLocations?.doc) {
    ...
    await addDocumentWithId(COLLECTION_NAME, userUid, {
      userUid,
      locations: [location],
    });
  }
...
export async function getUserRecentLocations(userUid: string) {
  const documents = await getDocumentsByUserUid<UserLocationData>(
    COLLECTION_NAME,
    userUid
  );
  ...
  if (documents && documents.length > 0) {
    console.warn(
      `Found ${documents.length} documents for user with UID: ${userUid}. Using first document.`
    );
  }
...
export async function removeRecentLocation(
  userUid: string,
