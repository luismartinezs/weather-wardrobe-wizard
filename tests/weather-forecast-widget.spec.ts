...
test('Weather forecast widget loads after the user clicks the "Get weather in {location}" button and shows 5 days', async ({ page }) => {
  await expect(page.locator('[data-testid="forecast-day"]')).toHaveCount(5);
});

test('Each day\'s forecast includes the necessary elements', async ({ page }) => {
